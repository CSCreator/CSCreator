...
from cscreator.components.componentcontroller import (
    ComponentController,
    EditableProperty,
    PropertyTypes,
    create_canvas,
...
class BoxController(ComponentController):
    ...
    def create(self, box_width_pixels: int, box_height_pixels: int) -> sg.SVGFigure:
        ...
        while pt_to_mm(text_width(text_to_render, fontsize=font_size)) >= width:
            font_size -= 1
            if font_size <= 1:
                break
            logger.info(
        ...
        text_w = pt_to_mm(text_width(text_to_render, fontsize=font_size))
        x = (width - text_w) / 2
        text.add(
            text.text(
